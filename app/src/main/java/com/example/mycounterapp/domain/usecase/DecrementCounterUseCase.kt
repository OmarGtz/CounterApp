 ...
 * @author (c) 2022
 */
class DecrementCounterUseCase @Inject constructor(val counterRepository: CounterRepository,@IoDispatcher dispatcher: CoroutineDispatcher): UseCase<String, List<Counter>>(dispatcher) {
    /**
     * Override this to set the code to be executed.
     */
    override suspend fun execute(parameters: String): List<Counter> {
        return counterRepository.decrementCounter(parameters)
    }
}